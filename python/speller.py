'''
    Next Step: Debug why words are printing with a newline after them. And why the Words in Dictionary for Small is showing 143091, when I only have 2 words in the small dictionary. What up? 
'''
...
if not loaded:
    printf(f"Could not load {dictionary}.")
    sys.exit(1)
...
# Spell Check each word in the file
while True:
    c = file.read(1)
    if not c:
        break
    
    # Allow alphabetical characters and apostrophes (for possessiveness)
